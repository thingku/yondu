		...
		<div class='main'>
			...
			<div class='row'>
				...
				<div class='col-md-9'>
					...
					$adjacents = 3;
					 ...
					 if(isset($_GET['sort'])){
					 		...
					 		if($sort==1){
						 		...
						 		$query = "CALL countArticle('1','".$startDate."','".$endDate."','none','".$user_id."','1')";
						 	
						 	}else if($sort==3){

						 		$catName=$_GET['catName'];

						 		$query = "CALL countArticle('1','none','none','".$catName."','".$user_id."','1')";

					 	 	}

					 }else{

					 	$query = "CALL countArticle('0','none','none','none','".$user_id."','1')";
					...
					if(isset($_GET['sort'])){
					 	...
					 	if($sort==1){
					 		...
					 		$startDate=$_GET['startdate'];
					 		$endDate=$_GET['enddate'];
					 		$sql = "CALL getArticle('".$user_id."','3','".$start."','".$limit."','1','".$startDate."','".$endDate."','none')";
				 		
				 		}else if($sort==3){

					 			$catName=$_GET['catName'];
						 		$sql = "CALL getArticle('".$user_id."','3','".$start."','".$limit."','2','none','none','".$catName."')";

				 		}

					 }else{

					 	$sql = "CALL getArticle('".$user_id."','3','".$start."','".$limit."','0','none','none','none')";
				...
				?>
					...
					?>
									<td><input type='checkbox' name='sub[]' value='<?php echo $getq["article_id"]; ?>'/></td>
									<td><a href='view_article.php?id=<?php echo $getq["article_id"]; ?>&tab=3&user_type=<?php echo $user_type; ?>&page=<?php echo $page; ?>' class='viewNews' ><?php echo $getq['title']; ?></a></td>
									...
									<td><a href='edit_news.php?id=<?php echo $getq["article_id"]; ?>&type=draft'>Edit</a></td>
						...
						?>
								...
								<input type="submit" class="btn btn-danger" name='deleteDraft' value='Delete'/>
