...
?>
	...
	<head>
		...
		<title>News | Submitted Articles</title>
	...
	<body>
		<form method='post' action='../backend/search.php?type=2'>
		<?php
			...
			<div class='row'>
				...
				<div class='col-md-9'>
					...
					<?php
						...
						$adjacents = 3;
						 ...
						 if(isset($_GET['sort'])){
						 	...
						 	if($sort==1){
						 		...
						 		$query = "CALL countArticle('1','".$startDate."','".$endDate."','none','".$user_id."','2')";
						 	 
						 	 }else if($sort==3){

						 		$catName=$_GET['catName'];

						 		$query = "CALL countArticle('5','none','none','".$catName."','".$user_id."','0')";

					 	 	}

						 }else{

						 	$query = "CALL countArticle('0','none','none','none','".$user_id."','2')";
						...
						if(isset($_GET['sort'])){
						 	...
						 	if($sort==1){
						 		...
						 		$startDate=$_GET['startdate'];
						 		$endDate=$_GET['enddate'];
						 		$sql = "CALL getArticle('".$user_id."','2','".$start."','".$limit."','1','".$startDate."','".$endDate."','none')";
					 		
					 		}else if($sort==3){

					 			$catName=$_GET['catName'];
						 		$sql = "CALL getArticle('".$user_id."','2','".$start."','".$limit."','2','none','none','".$catName."')";

				 			}
						 }else{

						 	$sql = "CALL getArticle('".$user_id."','2','".$start."','".$limit."','0','none','none','none')";
					...
					?>
									...
									<td><a href='view_article.php?id=<?php echo $getq["article_id"]; ?>&tab=2&user_type=<?php echo $user_type; ?>&page=<?php echo $page; ?>' class='viewNews' ><?php echo $getq['title']; ?></a></td>
									...
									?>	
											...
											<td><a href='edit_news.php?id=<?php echo $getq["article_id"]; ?>&editor=0&type=2'>Edit</a></td>
